...
int read_sfo_key(char* key, char* out, const char* sfo) {
	int ret = 0;
	...
	// get sfo header
	ret = read_sfo_key_buffer(key, out, sfo_buffer, sfo_size);
...
int read_sfo_key_buffer(char* key, char* out, const char* sfo_buffer, size_t sfo_size) {
	sfo_header* header = (sfo_header*)sfo_buffer;
	
	if(memcmp(header->magic, "\0PSF", sizeof(header->magic)) != 0) return -4; // check magic	
	if(header->count > 200) return -5; // give up if more than 200 keys
	if(sfo_size < (sizeof(sfo_header) + (sizeof(sfo_key) * header->count))) return -6; // check if size is enough for keys + sfo header size
	...
	{
		if(ptr > sfo_size) return -7; // check for overflow
		...
		if(name_offset > sfo_size) return -8;
		
		int data_offset = header->value_offset + s_key.data_offset;
		if(data_offset > sfo_size) return -9;
